...
export default function AI() {
    ...
    async function loadModelsAndRecommended() {
        ...
        const installed = Array.from(new Set(available ?? []));
        setModels(installed);
        setRecommended(curated ?? []);
        if (!selectedModel && installed.length > 0) {
            setSelectedModel(installed[0]);
        }
        if (selectedModel && installed.length > 0 && !installed.includes(selectedModel)) {
            setSelectedModel(installed[0]);
        }
    ...
    return (
        <div className="flex h-[calc(100vh-4rem)] w-full overflow-hidden text-white">
            <aside className="w-80 shrink-0 border-r border-neutral-800 bg-neutral-950 p-4 overflow-y-scroll">
                ...
                <div className="mt-4">
                    <label className="mb-2 block text-xs text-white/60">Quantized Model</label>
                    <select
                        value={selectedModel}
                        onChange={(e) => setSelectedModel(e.target.value)}
                        className="w-full rounded-lg border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm"
                    >
                        {models.length === 0 && <option value="">No quantized models installed</option>}
                        {models.map((m) => (
                ...
                <div className="mt-4 space-y-2">
                    <p className="text-xs text-white/60">Download & prepare models</p>
